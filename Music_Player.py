...
class MusicManager:
    ...
    def __init__(self, root):
        ...
        # Window Geometry - subject to change
        self.root.geometry("1000x200+200+200")
        # Initiate Pygame
        pygame.init()
        # Initiate Pygame mixer
        pygame.mixer.init()
        # Declare track variable
        self.track = StringVar()
        # Declare status variable
        self.status = StringVar()

        # Create track Fram for Song label & status label
        trackframe = LabelFrame(self.root, text="Song Track", font = ("helvetica", 15, "bold"), bg="grey", fg="white", bd=5, relief=GROOVE)
        trackframe.place(x=0, y=0, width=600, height=100)
        # Inserting Song Track Label
        songtrack = Label(trackframe, textvariable=self.track, width=20, font=("helvetica", 24, "bold"), bg="grey", fg="gold").grid(row=0, column=0, padx=10, pady=5)
        # Inserting Status Label
        trackstatus = Label(trackframe, textvariable=self.status, font=("helvetica", 24, "bold"), bg="grey", fg="gold").grid(row=0, column=1, padx=10, pady=5)

        # Creating Button Frame
        buttonframe = LabelFrame(self.root, text="Control Panel", font=("helvetica", 15, "bold"), bg="grey", fg="white", bd=5, relief=GROOVE)
        ...
        #TODO: Add error handling to handle non-compatible files(e.g. non-mp3s)
...
root = Tk()
# Passing Root to MusicManager Class
MusicManager(root)
# Root Window Looping
root.mainloop()